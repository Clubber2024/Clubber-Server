...
public interface RecruitRepository extends JpaRepository<Recruit,Long>{
    ...
    @Query("SELECT r FROM Recruit r LEFT JOIN r.recruitImages WHERE r.isDeleted = false ORDER BY r.id DESC")
    Page<Recruit> findRecruitsWithImages(Pageable pageable);

    @Query("SELECT r FROM Recruit r LEFT JOIN r.recruitImages WHERE r.club = :club AND r.isDeleted = false ORDER BY r.id DESC")
